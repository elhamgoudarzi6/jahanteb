...
export class ProductEditDialogComponent implements OnInit {
    ...
    this.service.getAllCategories().subscribe((response:any) => {
      if (response.success === true) {
        ...
        this.subCategories = response.data.filter(
          (x:any) => x._id === this.product.categoryID
        )[0].SubCategory;
  ...
  getDiscounts(): any {
    this.service.getAllDiscounts().subscribe((response:any) => {
      if (response.success === true) {
  ...
  onMultipleUpload(event:any): void {
    ...
    this.service.uploadMultiFiles(formData).subscribe((response:any) => {
      console.log(response);
      if (response.success === true) {
        let imgPathList: any[] = [];
        response.imagePath.forEach((element:any) => {
  ...
  addFeature(feature:any, value:any): void {
    let dup = false;
    if (feature != '' && value != '') {
