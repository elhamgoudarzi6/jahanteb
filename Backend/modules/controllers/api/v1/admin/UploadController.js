...
module.exports = new class UploadController extends Controller {
    ...
    uploadImage(req, res) {
        if(req.file) { 
            ...
            res.json({
                message : 'تصویر با موفقیت آپلود شد',
               imagePath : `http://api.jahantebkhoram.ir/` + req.file.path.replace(/\\/g , '/'),
                data :req.file,
                success : true,
            })
        } else {
    ...
    deleteFile(req, res) {
        ...
        try {
            // fs.unlinkSync('http://api.jahantebkhoram.ir' +((req.body.path).slice(21)).replace(/\\/gi, '/'));
            fs.unlinkSync(((req.body.path).slice(6)).replace(/\\/gi, '/'));
            return res.json({
