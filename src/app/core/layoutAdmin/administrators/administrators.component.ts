...
export class AdministratorsComponent implements OnInit {
  ...
  getAdministrators(): any{
    ...
    this.service.getAllAdmininstrators().subscribe((response:any) => {
  ...
  showEditAdminDialog(id: string): void {
    let admin = this.administrators.filter((x:any) => x._id == id)[0];
  ...
  deleteAdmin(id: any): void {
    this.confirmationService.confirm({
      ...
      accept: () => {
        // delete from db
        this.service.deleteAdmin(id).subscribe((response:any) => {
          if (response.success === true) {
